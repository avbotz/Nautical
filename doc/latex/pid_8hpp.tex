\hypertarget{pid_8hpp}{}\section{include/pid.hpp File Reference}
\label{pid_8hpp}\index{include/pid.\+hpp@{include/pid.\+hpp}}


Helper class to compute \hyperlink{structPID}{P\+ID} for motors.  


This graph shows which files directly or indirectly include this file\+:
\nopagebreak
\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[width=184pt]{pid_8hpp__dep__incl}
\end{center}
\end{figure}
\subsection*{Data Structures}
\begin{DoxyCompactItemize}
\item 
struct \hyperlink{structPID}{P\+ID}
\begin{DoxyCompactList}\small\item\em Helper class for \hyperlink{structPID}{P\+ID} computations. \end{DoxyCompactList}\end{DoxyCompactItemize}


\subsection{Detailed Description}
Helper class to compute \hyperlink{structPID}{P\+ID} for motors. 

\begin{DoxyAuthor}{Author}
David Zhang 
\end{DoxyAuthor}
